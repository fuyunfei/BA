...
class BALProblem {
 public:
  ...
  double* mutable_point_for_observation(int i) {
    return mutable_points() + point_index_[i] * 3;  // �˴� *3 ��Ϊ *1
  }
  bool LoadFile(const char* filename) {
    ...
    num_parameters_ = 9 * num_cameras_ + 3 * num_points_;  // 3-->1
...
//              }
          ...
          case 3:{ // xj= Px/Pz=Px*w=px/(-f) yj=Py/Pz=Py*w=py/(-f)
              const T& xj = point[0];
              const T& yj = point[1];
              const T& wj = point[2];
            // const T& focal = camera[6];
  ...
  static ceres::CostFunction* Create(const double observed_x,const double observed_y,
    ...
    return (new ceres::AutoDiffCostFunction<WjError, 2, 9, 3>(
...
int main(int argc, char** argv) {
  ...
  writeply(argv[2],bal_problem,focal_length);

  std::ofstream myfile;


  myfile.open ("../ply/ARTreport.txt");
